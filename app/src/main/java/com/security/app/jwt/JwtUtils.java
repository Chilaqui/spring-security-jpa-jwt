...
// Comprueba si el token ha expirado comparando la fecha de expiración con la fecha actual.
private boolean isTokenEspired(String token) {
    return extractEspiration(token).before(new Date()); 
    // Si la expiración es anterior a la fecha actual, el token ya expiró (retorna true).
}
...
// Extrae todos los claims (datos) del JWT. Un claim es un dato dentro del token.
